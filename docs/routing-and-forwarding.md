...
Examples:
- flb.homelab.docker.nginx.node-01
- flb.aws.node.log.systemd.ec2-worker-02
- flb.azure.node.log.auth.azure-vm-03
```

During ingestion filtering, the core Lua formatter ([`apply-standard-record-formatting.lua`](../docker/overlay/etc/fluent-bit/apply-standard-record-formatting.lua)) saves the full tag into the log record as `source_tag`. This ensures downstream storage backends (such as Loki or OpenObserve) retain complete auditability of the log's origin.
...
- **Real Log Matcher**: `^(?!.*_fmt\.).*` (or `${FLUENT_BIT_TAG_PREFIX}**`)
  - Matches all original, clean incoming logs.
  - Used by **STDOUT debug output**, **S3 cold storage**, **Plaintext Forward output**, and **TLS Forward output**.
...
flowchart LR
    ...
    subgraph RoutingPipeline [Output Routing & Format Rewriting]
        ...
        subgraph RewriteEngine [rewrite_tag Engine]
            ...
            --> D["<b>apply-loki-formatting.lua</b><br><small>Reshapes copy for Loki API</small>"]
