...
- **Enrich Records in Real Time**: Normalize Swarm service names (`stack_service.slot.taskid`), extract container IDs, classify log categories (`docker`, `system`, `auth`, `audit`), and inject environmental metadata (`source_env`, `source_env_type`, `source_env_region`, `source_hostname`, `source_env_isolation_scope`).
